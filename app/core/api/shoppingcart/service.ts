...
export const assignImageFromCart = async (
  user_id: number,
  image_id: number,
) => {
  try {
    const response = await orderApi.put(
      `/assignImageToCartByUserId/${user_id}/${image_id}`,
    );
    return response.data;
  } catch (error) {
    // console.error('Error fetching data:', error);
...
export const deleteAllImagesFromCart = async (user_id: number) => {
  try {
    const response = await orderApi.delete(
      `/deleteAllImagesFromCart/${user_id}`,
    );
    return response.data;
  } catch (error) {
    // console.error('Error fetching data:', error);
...
export const deleteImageFromCart = async (
  user_id: number,
  image_id: number,
) => {
  try {
    const response = await orderApi.delete(
      `/deleteImageFromCart/${user_id}/${image_id}`,
    );
    return response.data;
  } catch (error: any) {
    console.log(error.response.data.message);
